...
| `TURSO_DATABASE_URL` / `TURSO_AUTH_TOKEN` | **Yes**        | Present in `fly secrets list --app staktrakr` (verified 2026-08-13); `TURSO_DATABASE_URL` shares `SQLD_URL`'s digest. Read only if `SQLD_URL` is unset. Self-hosters on Turso Cloud set these instead. |
