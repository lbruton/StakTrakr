...
| Collections merge             | `collections-core.js` `mergeStates` via `cloud-sync.js` `_mergeCollectionState` | normalized state; per-Slot `modified`, Tombstones, `metaModified`; deterministic on ties             | commutative + idempotent   | STRK-370       |
