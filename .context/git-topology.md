...
- **Squash-slip recovery** — if a past `dev→main` squash froze the merge base (symptom: a `dev→main` PR conflicts on `sw.js` or balloons to a whole-tree diff): heal with a **zero-content** `git merge -s ours origin/dev` onto `main`, landed via a chore PR merged with `--merge`. It records the shared ancestry without changing `main`'s tree. Verify `git merge-base origin/main origin/dev == git rev-parse origin/dev`. `/ship` Steps 1.5 (pre-flight) and 6.6 (post-merge) automate this detect-and-heal.
- **GitHub limitation** — merge methods are repo-wide; "squash for feature→dev, merge-commit for dev→main" cannot be enforced structurally, so the squash button stays available at ship time. The `/ship` ancestry gates are the guard rail against a mis-click.
