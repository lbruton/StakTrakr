...
| **HOLD**      | every call site                                                                           | `_mergeCollectionState` **throws** on a failed write. `_applyAndFinalize` rolls back inventory + tags + Collections (`_snapshotCollectionState` / `_restoreCollectionState`) and returns without recording `lastPull`        |

**Push:** `collectionsStore.save()` calls `scheduleSyncPush()` (guarded, best-effort) because a Collections-only edit never reaches `saveInventory()`. The poll-level `computeSettingsHash` already covers the key logically, so only the manifest fast path needed the explicit re-detect.
...
**Test depth:** the contract is pinned by `tests/unit/cloud-sync-collection-state.test.js` and, at the `window.CloudSyncTest` seam, by `tests/playwright/core/collections-data-paths.spec.js`. There is **no mock-Dropbox end-to-end case** for the three apply chokepoints yet — they mirror the watermark call sites line for line.
