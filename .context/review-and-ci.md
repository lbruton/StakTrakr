...
## Review routing (verified 2026-08-13 — automatic, NOT label-gated)

AI reviewers **auto-review every `dev` PR, labels or not.** `.coderabbit.yaml` sets
`reviews.auto_review.base_branches: [dev]` with `drafts: true` and carries **no label
filter anywhere**; its own header comment records that the change was made to _enable_ dev
auto-review. Live confirmation: PRs #1441, #1442, and #1444 all merged with `labels: []`
and `coderabbitai` among their reviewers.

**Do not treat a missing AI review as expected.** Labels are not a gate; a silent reviewer
is a real gap worth investigating.

| Reviewer       | Trigger                                      | Notes                                                                                                                                                                                                                                                                                                                                                                                                               |
| -------------- | -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **CodeRabbit** | Automatic on every `dev` PR, drafts included | Enabled by `.coderabbit.yaml` `auto_review.base_branches: [dev]`, no label filter. Auto re-review is **paused after the first review** (incremental disabled) — follow-up commits don't trigger a fresh review; re-trigger manually. `request_changes_workflow: true` → a `CHANGES_REQUESTED` clears only on a clean re-review (a `COMMENTED` re-review does not flip it). ~4–8 reviews/hr throttle → 5–10 min lag. |
...
- Every `dev` PR is auto-reviewed by CodeRabbit, Codacy AI, Copilot, and Codex regardless of labels — budget for bot threads on every PR, including docs-only chores. (See "Review routing" above.)
...
- **A missing CodeRabbit review is NOT expected** — this entry previously claimed the opposite. CodeRabbit auto-reviews every `dev` PR regardless of labels (verified 2026-08-13 against `.coderabbit.yaml` and live PR data). If a `dev` PR has no CodeRabbit review, investigate rather than assuming it is by design. (See "Review routing" above.)
