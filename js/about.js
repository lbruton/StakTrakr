...
const getEmbeddedWhatsNew = () => {
  return `
    <li><strong>v3.34.85 &ndash; STRK-96: Playwright suite failures</strong>: Add Item tag controls now work in add mode by staging pending tags until the new item UUID exists, while the Playwright expectations were refreshed for current Goldback retail lookup dates, currency-rounded purchase inputs, and Numista description-field defaults (STRK-96).</li>
    <li><strong>v3.34.84 &ndash; STRK-101: manifest-first sync item edits</strong>: Cloud sync now normalizes manifest item change types before summary counts, diff classification, and conflict detection, preserving item field edits in manifest-first pulls; ZIP JSON plus CSV exports now carry frame and DIFF_FIELDS backup data needed for cross-device recovery (STRK-101).</li>
    <li><strong>v3.34.83 &ndash; STRK-99 hotfix: BE spot-ticker leak</strong>: After v3.34.82's denylist forced Bullion Exchanges through the markdown extractor, the spot ticker rendered inside the page header was being matched as the product price (silver $75.92 / gold $4,520 across every coin). Retail poller now strips <code>&lt;nav&gt;</code>, <code>&lt;header&gt;</code>, and <code>&lt;footer&gt;</code> blocks before plain-text extraction and uses a tier-anchored prose matcher (<code>1-49 $77.82</code>) so SDB and BE either land on the correct 1-unit Check/Wire price or report no price at all (STRK-99).</li>
