...
const getEmbeddedWhatsNew = () => {
  return `
    <li><strong>v3.35.80 &ndash; The header is now a single clean row</strong>: Eight buttons have left the top of the screen. Every one of them duplicated something you could already reach elsewhere, and each has moved to where it belongs: <strong>Backup</strong> and <strong>Restore</strong> to <strong>Settings &rsaquo; Storage</strong>, <strong>Cloud Sync</strong> to <strong>Settings &rsaquo; Cloud</strong>, <strong>Info</strong> to <strong>Settings &rsaquo; About</strong>, and <strong>Currency</strong> to <strong>Settings &rsaquo; Currency &rsaquo; Display currency</strong>. <strong>Trend</strong> and <strong>Spot Sync</strong> moved down onto the spot cards themselves &mdash; the period chip and the refresh icon on each card do the same job closer to the prices they affect. You will notice the currency one most, since it was the only button visible by default. Nothing you could do before has been taken away, and the header keeps <strong>Theme</strong> and <strong>Settings</strong> (STRK-283 to STRK-289, STRK-298).</li>
    ...
    <li><strong>v3.35.71 &ndash; Metal ratios, with an app of their own</strong>: The gold-to-silver ratio and its three companions &mdash; gold to platinum, gold to palladium, and silver to platinum &mdash; now have a proper home. Each spot card carries a small ratio chip, and clicking one opens a full panel with the current reading set against its own history, so you can see whether today is high or low by that measure rather than guessing. The same panel is also a standalone page at <strong>/ratios/</strong> that installs to your home screen as its own app with its own icon, separate from the main tracker &mdash; useful if ratios are what you check most often (STRK-268 to STRK-274).</li>
