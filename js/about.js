...
const getEmbeddedWhatsNew = () => {
  return `
    <li><strong>v3.35.96 &ndash; The footer health badge now tells the truth during a failover</strong>: StakTrakr publishes its price feeds to two independent servers, and when the primary falls behind the app quietly switches to the backup &mdash; but the little health badge in the footer only ever watched the primary, so during a brief hosting incident it warned &ldquo;2h ago&rdquo; in orange while the prices on screen were minutes old. The badge now reports the age of whichever server is actually supplying your data, and a small status icon in front of it watches the infrastructure: &#9989; when both servers are healthy, &#9888;&#65039; when one is stale or unreachable. So a failover now reads as a green, current timestamp with a warning icon &mdash; your data is fine, one feed is having a moment &mdash; and clicking the badge still opens the full health breakdown explaining exactly which server is behind (STRK-331).</li>
