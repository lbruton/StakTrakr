...
const getEmbeddedWhatsNew = () => {
  return `
    ...
    <li><strong>v3.35.38 &ndash; STRK-221: Image-rule errors no longer silent</strong>: When you add or edit a custom image pattern rule and the chosen image can't be processed, StakTrakr now shows a clear error and stops, instead of quietly saving a rule with no picture. You'll know right away to pick a different file, so you won't end up with empty image rules that never display anything (STRK-221).</li>
    <li><strong>v3.35.37 &ndash; STRK-226/227: Backup &amp; CSV-export cleanup</strong>: Internal maintenance only &mdash; nothing changes in your data, your exports, or anything you see. We removed an unused legacy backup code path and merged the duplicated CSV value-column logic shared by the inventory CSV export and the backup ZIP into one helper, so a fix to one exporter can no longer silently miss the other. Your CSV and backup files are byte-for-byte the same as before (STRK-226, STRK-227).</li>  `;
};
