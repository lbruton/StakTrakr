...
const getEmbeddedWhatsNew = () => {
  return `
    <li><strong>v3.35.50 &ndash; Constitutional / junk silver tracking (90% / 40% / 35%)</strong>: A new first-class <strong>Constitutional</strong> type for U.S. junk silver. Add holdings by denomination and coin count (90% dimes, quarters, halves, Morgan/Peace dollars, 40% Kennedy halves and Eisenhower dollars, 35% war nickels) or by total face value, and StakTrakr derives the real silver content and melt value automatically &mdash; silver dollars valued at their true ~0.7734&nbsp;ozt rather than the subsidiary rate, so you never hand-calculate weight. A worn-vs-fresh valuation basis toggle in Settings &rarr; Currency reprices the whole portfolio at once (STRK-235).</li>
    ...
    <li><strong>v3.35.51 &ndash; Bulk-convert holdings to constitutional silver</strong>: The Bulk Edit tool can turn a batch of items into junk silver &mdash; enable Type, choose Constitutional, pick a denomination, and apply, and each item gets its correct derived silver content (keeping its existing quantity as the coin count) instead of the $0 melt value bulk conversion used to produce (STRK-238).</li>
    ...
    <li><strong>v3.35.49 &ndash; Reliable two-session cloud sync</strong>: Fixed a re-entrancy race where two browsers syncing at the same time could fail with &ldquo;Could not decrypt vault for preview&rdquo; on an empty-diff pull. Overlapping pull cycles are now serialized so they can't interleave on shared state (STRK-234).</li>
  `;
};
