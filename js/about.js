...
const getEmbeddedWhatsNew = () => {
  return `
    <li><strong>v3.36.5 &ndash; STRK-306: Copper arrives on the dashboard</strong>: The final piece of copper support &mdash; a copper spot price card and a copper summary card, off by default and switched on in Settings &rarr; Appearance &rarr; Metal Order. The spot card quotes copper the way the copper market does, in dollars per pound (about $6), with the per-troy-ounce figure underneath &mdash; a forty-one-cent number next to gold&rsquo;s four thousand would look broken, even though that per-ounce value is exactly what&rsquo;s stored and used for your melt math. Enabling copper also reflows the dashboard: six summary cards fit in one row on desktop with labels stacked above the values so a six-figure portfolio never gets cut off with &ldquo;&hellip;&rdquo;, and on phones your first-ordered metal now gets the full-width top slot. Leave copper off and nothing changes at all. One small default: the All Metals card now leads the summary row for new users &mdash; your own saved order is untouched (STRK-306).</li>
