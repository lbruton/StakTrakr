...
const getEmbeddedWhatsNew = () => {
  return `
    <li><strong>v3.35.84 &ndash; STRK-299/300: Junk silver leads with face value, and the silver figure is called ASW</strong>: The Weight column for constitutional (junk) silver rows now reads <strong>$6.00 fv</strong> &mdash; the total face value of the lot &mdash; rather than the derived silver ounces. Face value is how junk silver is quoted, bought and talked about, and the table was the one place that did not lead with it; the card views and the item detail window already did. The silver content moves to the cell tooltip along with the worn/fresh basis, and it now has a proper name: <strong>ASW</strong>, or Actual Silver Weight, the term dealers use when quoting bags. The app used ASW internally but never showed it, and the three places displaying that number each called it something different. The detail window also stops labelling a dollar amount as a &ldquo;Weight&rdquo; &mdash; it says <strong>Face value</strong>, with the ASW on its own row below. Sorting and filtering are unchanged: the Weight column still ranks by silver content so junk silver interleaves correctly with your bullion, which is why $1.00 of war nickels sits above $1.25 of 90% dimes. Melt values, weight totals and CSV exports are identical, and face value is never currency-converted (STRK-299, STRK-300).</li>
