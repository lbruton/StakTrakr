...
const getEmbeddedWhatsNew = () => {
  return `
    <li><strong>v3.35.30 &ndash; STRK-201: Faster backup export for large inventories</strong>: Creating a backup ZIP no longer slows down as your collection grows &mdash; the per-photo lookup that matches each saved image to its item now uses an indexed lookup instead of re-scanning the whole inventory for every photo. The backup contents are unchanged; only the export speed improves, most noticeably for users with many items and many saved photos (STRK-201).</li>
    <li><strong>v3.35.29 &ndash; STRK-211/212: Export crash fix for incomplete items</strong>: Exporting a CSV or creating a backup no longer crashes when an item is missing its metal or has a non-numeric weight &mdash; those items now fall back to "Silver" and a 0.0000 weight in the output instead of aborting the entire export. This protects exports of older or partially-filled entries (STRK-211, STRK-212).</li>
    <li><strong>v3.35.28 &ndash; STRK-200: User-image restore orphan guard</strong>: Restoring a backup no longer brings in per-Item photos for items that are not in your inventory &mdash; both the unencrypted ZIP and the encrypted vault/cloud restore paths now skip them, just as attachments already did. This stops orphaned images from quietly piling up in storage and appearing as un-editable entries in the Settings Per-Item User Images list (STRK-200).</li>
    <li><strong>v3.35.27 &ndash; STRK-202: Pattern-rule images survive restore</strong>: Your custom pattern-rule images now travel inside backups &mdash; restoring a backup re-binds them to their rules instead of orphaning them, the loss a beta user hit after a restore. A new Settings panel also lists and clears any already-orphaned pattern images to reclaim space (STRK-202).</li>
    <li><strong>v3.35.26 &ndash; STRK-147: Cross-device price history sync</strong>: Your per-Item price history now syncs across all your devices whenever Cloud Sync is enabled. Entries recorded on different devices merge together by union &mdash; never overwriting one another &mdash; through a new always-on encrypted companion vault, and the merge happens quietly in the background with no review prompt. Your existing local history, manual backups, and retention limits are unchanged (STRK-147).</li>  `;
};
