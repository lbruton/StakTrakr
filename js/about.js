...
const getEmbeddedWhatsNew = () => {
  return `
    <li><strong>v3.35.95 &ndash; STRK-328: The Inventory tab can no longer be hidden</strong>: Inventory now sits alongside Dashboard as an always-visible tab, and its checkbox in Settings &rsaquo; Appearance &rsaquo; Layout is greyed out. It was never really optional in the way Market and Collections are: Inventory holds the only button that adds or edits an item, and everything on the Dashboard is worked out from what you have entered there. Hiding it left you looking at a page of zeroes with no way to add anything, and the only escape was to remember which setting had done it &mdash; from the outside the app simply looked broken. If you had already hidden it, the tab and its contents come back on their own the next time you open the app; you do not need to change anything. Market and Collections are still yours to hide, and the Dashboard&rsquo;s own spot cards, ticker, and totals can still be switched off individually if you want a leaner first screen (STRK-328).</li>
