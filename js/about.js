...
const getEmbeddedWhatsNew = () => {
  return `
    <li><strong>v3.35.62 &ndash; Goldback intraday price history</strong>: Goldback now publishes a dedicated hourly price feed covering the last three days, and the goldback &ldquo;latest&rdquo; price is stamped with the real scrape hour instead of a midday placeholder &mdash; so a future intraday goldback chart has real data and the timestamp no longer pretends to be a once-a-day snapshot. The goldback price also refreshes on a 2-hour budget instead of being cached for up to a day (STRK-248).</li>
    <li><strong>v3.35.61 &ndash; Realtime prices now load fresh on every visit</strong>: Spot, goldback, and retail prices are served straight from the network instead of a stale cached copy, so the gold card's goldback badge and the market premiums show current values on a normal page load &mdash; no more hard refresh to see today's rate. Price lookups also fall back to the backup API automatically when the primary is unreachable (STRK-249).</li>
    <li><strong>v3.35.60 &ndash; Lot pricing for junk silver by denomination</strong>: When you add constitutional (junk) silver &ldquo;by denomination,&rdquo; the purchase-price Lot/Each toggle now appears and defaults to Lot, so what you paid for the whole lot is divided by the coin count instead of being stored as the per-coin price. By-face-value entries stay a single lot-of-one with no division, and editing an item reopens in the mode you saved it in. Your cost basis and totals line up, and the lot/each choice now survives JSON export, ZIP backup, and cloud sync (STRK-242).</li>
    <li><strong>v3.35.59 &ndash; Custom purity no longer lost behind a hidden field</strong>: Setting a Custom purity and then switching an item's Type to Constitutional and on to Goldback or Silverback before saving used to hide the custom-purity field while still quietly saving its old value. The field now re-appears whenever your purity is still &ldquo;custom&rdquo; (and the Type isn't Constitutional), so what you see is exactly what gets saved &mdash; no stale purity hiding off-screen (STRK-247).</li>
