...
async function loadSeedSpotHistory() {
  ...
  debugLog(
    "Seed data: " +
      (isMerge
        ? "existing runtime history detected (" + existing.length + " entries)"
        : "loading for first-time user...")
  );
  ...
  if (typeof historicalDataCache !== "undefined" && historicalDataCache.size > 0) {
    ...
    debugLog(
      "Seed data: loaded " +
        seedEntries.length +
        " entries from bundle cache (" +
        historicalDataCache.size +
        " years)"
    );
  }
