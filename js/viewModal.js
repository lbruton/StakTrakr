...
async function loadViewNumistaData(item, container, apiResult) {
  ...
  if (
    apiResult &&
    typeof showResyncPicker === "function" &&
    typeof window.applyPickerSelections === "function"
  ) {
...
function _valuesMatch(current, incoming) {
  ...
  if (Array.isArray(current) && Array.isArray(incoming)) {
    return current.length === incoming.length && current.every((v, i) => v === incoming[i]);
  }
...
function showResyncPicker(item, normalizedData, onConfirm, onCancel) {
  ...
  function _buildFieldRow(fieldName) {
    ...
    if (incomingDisplay) {
      ...
      valSpan.textContent =
        typeof sanitizeHtml === "function" ? sanitizeHtml(incomingDisplay) : incomingDisplay;
    ...
    if (!checked && !disabled && hasCurrentValue && !valuesEqual) {
      ...
      diffHint.textContent =
        "Current: " +
        (typeof sanitizeHtml === "function" ? sanitizeHtml(currentDisplay) : currentDisplay);
  ...
  if (tier2Container.childNodes.length > 0) {
    ...
    toggleBtn.textContent = _resyncPickerShowMore
      ? "\u25BE Hide extra fields"
      : "\u25B8 Show more fields";
    toggleBtn.addEventListener("click", function () {
      _resyncPickerShowMore = !_resyncPickerShowMore;
      tier2Container.style.display = _resyncPickerShowMore ? "block" : "none";
      this.textContent = _resyncPickerShowMore
        ? "\u25BE Hide extra fields"
        : "\u25B8 Show more fields";
    });
