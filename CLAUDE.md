...
- **Stale dev-targeting branches** → `/pr-cleanup` only detects `[gone]` refs, which requires the upstream branch to have been deleted. Squash-merged branches targeting `dev` never appear as `[gone]` (the local branch tracks `origin/dev`, not its merged ref) — so `git branch -vv | grep ': gone]'` will NOT find them. Cross-check instead: `gh pr list --state merged --search "head:patch/"` and compare against `.worktrees/patch-*/` plus `git for-each-ref --merged origin/dev refs/heads/`.
