...
63 JS files load in strict dependency order via `index.html` (plus 7 vendor libs and 1 data bundle = 71 total external `<script>` tags). Breaking this order causes undefined variable errors. The full chain:
...
inventory-backup.js -> inventory-import.js -> inventory-table.js -> inventory.js
card-view.js
...
| `chart-utils.js` | `createSparkline()`, `buildVendorDatasets()`, `createTimeSeriesChart()`, `replaceChart()`, `chartPriceTooltip()`, `chartDollarTicks()` |
| `inventory-backup.js` | `createBackupZip()`, `restoreBackupZip()`, `generateBackupHtml()` |
| `inventory-import.js` | `importCsv()`, `importJson()`, `exportCsv()`, `exportNumistaCsv()`, `showImportDiffReview()` |
| `inventory-table.js` | `renderTable()`, `updateSummary()`, `hideEmptyColumns()`, `recalcItem()`, `persistInventoryAndRefresh()`, `METAL_COLORS`, `METAL_TEXT_COLORS` |
| `inventory.js` | `saveInventory()`, `loadInventory()` |
...
5. Create worktree + branch: `git worktree add .worktrees/patch-VERSION -b patch/VERSION`
6. Do all work inside `.worktrees/patch-VERSION/`
7. Push branch → open draft PR `patch/VERSION → dev` (Cloudflare generates preview)
8. QA preview → merge to dev
9. Cleanup: `git worktree remove .worktrees/patch-VERSION --force && git branch -d patch/VERSION` — then remove **only your claim entry** from `devops/version.lock` (leave other active claims intact)
...
- `.worktrees/` is gitignored — worktree content dirs are transient
...
- **`js/chart-utils.js`** -- Shared Chart.js configuration builders, dataset factories, sparkline creator, tooltip/tick formatters (STAK-484 extraction)
...
- **`js/inventory-backup.js`** -- ZIP backup/restore, HTML backup generation (STAK-484 extraction from inventory.js)
- **`js/inventory-import.js`** -- CSV/JSON import/export, Numista CSV, import progress UI (STAK-484 extraction from inventory.js)
- **`js/inventory-table.js`** -- Table rendering, summary, color maps, column visibility, recalcItem (STAK-484 extraction from inventory.js)
...
StakTrakr documentation lives in **DocVault** (Obsidian vault) at `/Volumes/DATA/GitHub/DocVault/Projects/StakTrakr/`. This is the single source of truth for codebase architecture and patterns. Reference it before making architectural changes. Pages are maintained by agents — do not let docs drift after meaningful patches.
...
Architecture, data pipelines, Fly.io, pollers, secrets — see `DocVault/Projects/StakTrakr/` for full index.

### Documentation update policy

- Use `/vault-update` after any patch that changes JS, CSS, skills, or devops files
- DocVault updates MUST be committed BEFORE pushing or creating a PR
- Pages live at `DocVault/Projects/StakTrakr/*.md` in the DocVault repo

## Documentation Policy

DocVault (`/Volumes/DATA/GitHub/DocVault/Projects/StakTrakr/`) is the single source of truth for all architecture, operational runbooks, and pattern documentation. New documentation goes in DocVault (or `docs/plans/` for planning artifacts).
...
- Do not add new files without updating `sw.js` CORE_ASSETS and `index.html` script order — the 71-script dependency chain is critical
...
- Do not suggest lazy-loading for scripts in `index.html` — all 71 script files use `defer` (except `file-protocol-fix.js`) and load order is a hard dependency chain
...
- Do not remove functions that appear unused without checking global scope — functions defined in one file are called from files loaded later in the script order. Use the 71-script dependency chain (section 1 above) to trace callers before flagging dead code
...
The following MCP servers are available. Availability can vary by environment.
...
| `spec-workflow` | ✅ reachable | `mcp__spec-workflow__spec-list` |
...
- `spec-workflow`: Spec-driven development orchestrator — manage specs, approvals, task lists, and implementation logs.
  Typical flow: `spec-list` -> `spec-status` -> `approvals` -> `log-implementation`.
...
### MCP Agent Parity (as of 2026-03-21)
...
**Issue tracking** uses DocVault vault-based issues (prefix `STAK-`), not Linear. Reference issues as `STAK-###` in commits and PRs.
...
1. Preserve safety controls:
   ...
   - Keep secret-handling rules unchanged (no raw secrets in issue trackers; mem0 secret storage only
...
1. Keep state durable:
   - For non-trivial relayed work, write both an issue/PR handoff comment and a mem0 `add_memory` entry
     (or explicitly state why one is skipped).

## Cross-Agent Handoff Protocol (DocVault Issues + mem0)

Use this when both agents are working the same PR or issue.

### Goals

- Keep humans informed via DocVault issue comments and GitHub PR threads.
...
Use this short template in the related DocVault issue or GitHub PR:
...
After posting the issue comment, save to mem0:

```javascript
mcp__mem0__add_memory({
  text: "StakTrakr handoff to <agent>: <topic>. Issue: <STAK-###>. <summary of scope, next steps>",
  metadata: {
    project: "staktrakr",
    category: "workflow",
    type: "handoff"
...
- Always post issue comment + mem0 `add_memory` together for handoffs.
...
- Never include secrets in issue trackers or mem0 — use Infisical references only.
